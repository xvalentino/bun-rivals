...
import { HeroSchema, PlayerResponseSchema } from "@/lib/schema";
import { useQueries } from "@tanstack/react-query";
import { useState } from "react";
import { z } from "zod";

export const usePlayerAndHeroQueries = (playerName: string) =>
	useQueries({
		combine: (results) => {
			return {
...
export function PlayerStats({ playerName }: PlayerStatsProps) {
	...
	const hasUnrankedHeroes =
		heroesUnranked && Object.keys(heroesUnranked).length > 0;
			...
			<CardContent>
				...
				>
					<TabsList className="mb-4">
						...
						{hasRankedHeroes && (
							<TabsTrigger value="ranked-heroes">Ranked Heroes</TabsTrigger>
						)}
						{hasUnrankedHeroes && (
							<TabsTrigger value="unranked-heroes">Unranked Heroes</TabsTrigger>
						)}
					</TabsList>
