...
export function HeroesTab({
	data,
	playerName,
}: { data: { heroes: HeroesUnranked }; playerName: string }) {
	const { heroes } = data;
	...
	const { heroes: allHeroes, isLoading: isLoadingHeroes } =
		usePlayerAndHeroQueries(playerName);
									...
									<TableCell className="flex items-center justify-center">
										<Avatar>
											<AvatarImage
												...
												src={`https://marvelrivalsapi.com/${allHeroes?.find((h) => Number(h.id) === hero.hero_id)?.imageUrl}`}
											/>
										</Avatar>
									</TableCell>
